export const ResultsMap = ({ player, totalHealings, totalDam, totalKillings, characters }) => {
   ...
   return <div className="player__results"><div className="player__name">{rightCharacter?.character}</div><div className="damage">{player?.damage ? <>{percentage(player?.damage, totalDam)}%</> : "0"}
   </div>
